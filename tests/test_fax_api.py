...
def test_send_fax_success(monkeypatch, tmp_path):
	...
	def fake_post(*args, **kwargs):
		return FakeResponse(
			status_code=200,
			payload={"success": True, "data": {"id": 42}},
		)
...
def test_get_fax_status_success_mapping(monkeypatch):
	def fake_get(*args, **kwargs):
		return FakeResponse(status_code=200, payload={"data": {"status": "delivered"}})
...
def test_get_fax_status_in_progress_mapping(monkeypatch):
	def fake_get(*args, **kwargs):
		return FakeResponse(status_code=200, payload={"data": {"status": "queued"}})
